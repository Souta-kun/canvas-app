import { KanbanState, KanbanAction, Board } from "./types";
...
function updateActiveBoard(state: KanbanState, updater: (board: Board) => Board): KanbanState {
...
export function kanbanReducer(state: KanbanState, action: KanbanAction): KanbanState {
  switch (action.type) {
    ...
    case "ADD_BOARD":
      return {
        ...state,
        boards: [...state.boards, { id: action.payload.id, name: action.payload.name }],
      };
    ...
    case "UPDATE_BOARD": {
      ...
      if (state.activeBoard?.id === action.payload.id) {
        updated.activeBoard = { ...state.activeBoard, name: action.payload.name };
      }
      return updated;
    }
